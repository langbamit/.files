...
keymap._execute = function(id, _name)
  keymap._store[id]()
end
...
local make_mapper = function(mode, defaults, opts)
  ...
  elseif type(rhs) == 'function' then
    ...
    local as_cmd = map_opts.as_command
    if type(as_cmd) == 'string' then
        map_opts.as_command = nil
        vim.cmd(string.format(
            [[command! %s lua vim.keymap._execute(%s)]], as_cmd, func_id
        ))
        mapping = '<cmd>' .. as_cmd .. '<CR>'
    else
